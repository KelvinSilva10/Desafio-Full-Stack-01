...
interface iModalUpdateContactProps {
  isOpen: boolean;
  onClose: () => void;
  contactId: string;
...
const updateContactSchema = yup.object().shape({
  ...
  secondaryEmail: yup
    ...
    .max(50, 'Máximo de 50 caracteres')
    .optional(),
  mainPhone: yup.string().max(20).notRequired(),
  secondaryPhone: yup.string().max(20).notRequired(),
});
...
export const ModalUpdateContact = ({
  ...
  const handleUpdateContact = (data: iContactData) => {
    // const newData = { ...data, userId: user.id };
    updateContact(contactId, data, accessToken).then((res) => onClose());
  };
        ...
        <ModalBody textAlign='center'>
          <VStack spacing='5'>
            ...
            <Input
              ...
              defaultValue={contact.firstEmail}
            />
            <Input
